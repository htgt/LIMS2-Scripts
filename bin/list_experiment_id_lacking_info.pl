...
foreach my $item (@gene_ids)
{
    ...
    foreach my $project (@projects)
    {
        ...
        while (my $exp = $exp_hash_ref->next){
            ...
            compress_data($exp, $gene_info->{'gene_symbol'}, $csv, );
        }
    }
}

sub compress_data{
    my ($exp_data, $gene_symbol, $csv_file) = @_;
    foreach my $crispr ($exp_data->{'crisprs'}){
        for (my $i = 0; $i<4; $i++){
            print "\n Gene_Symbol: ", $gene_symbol, " Experiment_id: ", $exp_data->{'id'}," Crispr: ", $crispr->[$i]->{'id'}, " Seq: ", $crispr->[$i]->{'seq'}, "\n";
            write_data($gene_symbol, $exp_data->{'id'}, $crispr->[$i]->{'id'}, $crispr->[$i]->{'seq'}, $csv_file);
        }
    }
    return
}
...
sub write_data{
    my ($gene_symbol, $exp_id, $crispr, $seq, $csv_f) = @_;
    my $fh;
