...
sub find_lost_wells {
    ...
    foreach (my $i = 1; $i < 385; $i++) {
        if (my $lost_well = check_for_lost_well($plate_name, $i, $miseq_exp->{name}, $existing_well_names)) {
...
sub get_well_names {
    ...
    my @well_names = map {$_->well->name} $miseq_well_exp_rs->search({ miseq_exp_id => $miseq_exp_id});
    return \@well_names;
}
...
sub construct_file_path {
    ...
    my $file_path = "$quant_file_root/S${i}_exp$miseq_exp_name/CRISPResso_on_${i}_S${i}_L001_R1_001_${i}_S${i}_L001_R2_001/Quantification_of_editing_frequency.txt";
    return $file_path;
}
