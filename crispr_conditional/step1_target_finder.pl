...
sub print_targets {
    ...
    for my $exon_id ( keys %{ $insertion_sites } ) {
        ...
        $exon_params{exon_size} = $insertion_sites->{$exon_id}{exon}->length;
        ...
        $exon_params{exon_start} = $insertion_sites->{$exon_id}{exon}->start;
        $exon_params{exon_end} = $insertion_sites->{$exon_id}{exon}->end;
        ...
        for my $ins_site ( keys %{ $insertion_sites->{$exon_id}{sites} } ) {
            ...
            if ($exon_params{chr_strand} eq '-1') {
                $target_params{insertion_site} = $ins_site + 1;
            } elsif ($exon_params{chr_strand} eq '1') {
                $target_params{insertion_site} = $ins_site + 3;
            }

            $target_params{split_exon_L} = $target_params{insertion_site} - $exon_params{exon_start} + 1;
            $target_params{split_exon_R} = $exon_params{exon_end} - $target_params{insertion_site};
...
sub get_intron_cassette_insertion_sites {
    ...
    for my $exon ( @{ $exons } ) {
        ...
        # either side
        if ( %sites ) {
            $valid_sites{$exon->stable_id}{exon} = $exon;
